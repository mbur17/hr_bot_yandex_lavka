...
$(document).on('shown.bs.modal', '#modal-delete', function (event) {
  ...
  $("#modal-delete-text").text("Это действие безвозвратно удалит " + name + " " + pk + " ?");
...
$(document).on('click', '#modal-delete-button', function () {
  $.ajax({
    ...
    error: function (request, status, error) {
      console.log(request)
      // $("#modal-delete-info").text(request.responseJSON.message) 
      $("#modal-delete").modal("hide");
      $('#modal-delete-message').text(request.responseJSON.message);
      $('#modal-delete-info').modal('show');
